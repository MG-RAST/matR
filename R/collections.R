...
setMethod ("views", "collection", function (x) viewnames (x))				# should have its own pretty printing
...
setMethod ("$", "collection", function (x, name) as.matrix (x@views [[name]]))     # should drop attributes; does it?
setMethod ("$<-", "collection", function (x, name, value) { x@views [[name]] <- value ; x })
...
setMethod ("[", "collection", function (x, i) {
	x@sel <- x@sel [i]
	x@views <- lapply (x@views, `[`, i=, j=i)      # after this, attributes also may need to be adjusted
	x
	})
...
setMethod ("collection", "selection", function (x, ...) {
	...
	lapply (names (views), function (x) res [[x]] <<- views [[x]])
